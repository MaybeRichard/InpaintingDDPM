...
class Diffusion:
    ...
    def sample(self, model, disc, epoch, n):
        ...
        save_images(x, os.path.join("/data2/xiaoyi/DR_MA/result", f"{self.args.save_path}/{counter}_fake.png"))
...
def train(args):
    ...
    logging.basicConfig(level=logging.INFO,
                        ...
                        filename=None)
    device = args.device
    data_paths = glob("/data2/xiaoyi/DR_MA/OIA-DDR/MA_seg/All/image_1024_jpg/*.jpg")
    label_paths = glob("/data2/xiaoyi/DR_MA/OIA-DDR/MA_seg/All/mask_1024_jpg_fused_all/*.jpg")
    ...
    ddpm = DDPM(img_channels=1, time_dim=args.emb_dim,  device=args.device,out_channel=1).to(device)  # channels = 1
    optimizer = optim.AdamW(ddpm.parameters(), lr=args.lr, weight_decay=0.01)

    if args.load_model:
        load_checkpoint(
            os.path.join(args.checkpoints, "ddpm97.pth.tar"), ddpm, optimizer, args.lr, args.device
        )
    ...
    diffusion = Diffusion(noise_steps=800, img_size=args.image_size, device=device, args=args)
    ...
    for epoch in range(1, args.epochs):
        ...
        for i, (_, labels) in enumerate(pbar):
            ...
            # if i >1:
                ...
                diffusion.sample(ddpm, disc, epoch, n=2)  # score =
...
if __name__ == '__main__':
    ...
    if training:
        ...
        args.num_workers = 4
        args.save_path = "experiment_8_noise2_fused_all"
        args.checkpoints = "/data2/xiaoyi/DR_MA/results/experiment_8_noise2_fused_all/checkpoints"
